...
|---|---|---|
| Refill "succeeds" in ~1 second, no branch/PR created; run log shows `is_error: true`, `num_turns: 1`, `total_cost_usd: 0` | Claude's first model call was rejected instantly — almost always a `--model` slug the action's Claude CLI can't resolve (a full dated id like `claude-opus-4-8` that's newer than the installed CLI). | Use a CLI alias (`opus`/`sonnet`/`haiku`) for the `model` input, not a dated id. Fixed in engine `v1` (default is now `opus`). The "Fail if Claude errored" guard now turns this into a red run instead of a fake-green one. |
