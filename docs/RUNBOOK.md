...
|---|---|---|
| Refill fails fast; run log shows `is_error: true`, `num_turns: 1`, `total_cost_usd: 0`, identical across different `--model` values | Claude authenticated (init succeeded) but the account can't serve headless inference — an **entitlement problem with the `CLAUDE_CODE_OAUTH_TOKEN`**, not the model or the pipeline. Seen when a subscription token doesn't cover GitHub-Actions/headless use. | First, regenerate the token on the right account: `claude setup-token`, replace the secret, re-dispatch. If it still fails, switch auth: add an `ANTHROPIC_API_KEY` secret (console.anthropic.com) — the engine uses it in preference to the OAuth token. API-key auth is metered but is the reliable CI path. The "Fail if Claude errored" guard makes this a red run, not a fake-green one. |
...
| Refill fails with `Claude Code is not installed on this repository. Please install the Claude Code GitHub App` | The Claude GitHub App isn't installed on the repo — separate from the token. The token authorizes Claude; the App lets it run in the repo's Actions. Both are required (one-time). | Install it at https://github.com/apps/claude (grant this repo), or run `claude` locally → `/install-github-app`. Then re-dispatch. |
...
| Refill/audit fails with `Could not fetch an OIDC token. Did you remember to add id-token: write?` | `claude-code-action` uses OIDC internally to verify the OAuth token exchange, even though you're not using workload identity federation. Both the reusable workflow AND the caller workflow's `permissions:` block need `id-token: write`. | Already fixed in engine `v1` (2026-07-03) and in `caller-templates/`. If your site's caller workflow predates that fix, add `id-token: write` under its top-level `permissions:` and re-dispatch. |
